...
function concatenateStrings(value1, value2) {
  const concatenated = value1 + value2;
  return concatenated;
}
...
function isPalindrome(str) {
  const toLowerCased = str.toLowerCase();
  const reversed = [...toLowerCased].reverse();

  const filteredArr1 = reversed.filter((letter) => /[a-z]/.test(letter));

  const arr = toLowerCased.split('');
  const filteredArr2 = arr.filter((letter) => /[a-z]/.test(letter));
  return filteredArr1.join('') === filteredArr2.join('');
}
...
function extractNameFromTemplate(value) {
  ...
  arr.shift();
  const subStr = arr.join(' ');
  const newStr = subStr.substring(0, subStr.length - 1);
  return newStr;
}
...
function encodeToRot13(str) {
  const firstHalf = 'abcdefghijklmnopqrstuvwxyz';
  const secondHalf = 'nopqrstuvwxyzabcdefghijklm';

  return str
    .split('')
    .map((el) => {
      const lowerCaseEl = el.toLowerCase();
      if (firstHalf.includes(lowerCaseEl)) {
        const index = firstHalf.indexOf(lowerCaseEl);
        return el === lowerCaseEl
          ? secondHalf[index]
          : secondHalf[index].toUpperCase();
      }
      return el;
    })
    .join('');
